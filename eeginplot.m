%eeginplot() - Highlight segments with the mouse and select channels for interpolation.
%
% This function is a modified version of the original `eegplot()` function 
% from EEGLAB. The modifications allow the user to highlight segments and select 
% specific channels to interpolate. The changes made are:
%
% 1. Changed the `g.setelectrode` default value from 0 to 1 (Line 274).
% 2. Updated the figure tag from `EEGPLOT` to `EEGINPLOT`.
% 3. Modified the `winrej` variable handling:
%    - Each row in `g.winrej` represents a highlighted segment:
%      - Columns 1-2: Start and end points (in data points).
%      - Columns 3-5: RGB color for the highlight.
%      - Columns 6 onward: Logical vector indicating selected channels (1 = selected).
%    - Access `winrej` via the `g` structure stored in the figure's `UserData`.
%      Example:
%      ```matlab
...
%      winrej = g.winrej;
%      ```
% 4. Renamed the "Reject" button to "Interpolate" and updated its callback:
%    - Removed the condition `if ~isempty(g.command)` to always create the button.
%    - Replaced the `acceptcommand` with an `interpolation_command` that returns 
%      the `winrej` values to the workspace.
% 5. Added a new function `WinRej` to extract `winrej` into the workspace.
%
% Copyright (C) 2001 Arnaud Delorme & Colin Humphries, Salk Institute, arno@salk.edu
% Modified in 2024 by S.M. Ali Mousavi, smousavi@uvt.nl
%
% License:
...
%
% Acknowledgements:
% - Original authors: Arnaud Delorme & Colin Humphries (EEGLAB).
% - Modified by: S.M. Ali Mousavi (2024).
%
...
function [outvar1] = eeginplot(data, varargin); % p1,p2,p3,p4,p5,p6,p7,p8,p9)
...
%        % Check  consistency of freqs
  ...
  defctrlupcom = ''; % CTRL press and up -> do nothing by default
   ...
   try, g.tag; 				catch, g.tag		= 'EEGINPLOT'; end;
...
%    try, g.butlabel; 		catch, g.butlabel   = 'REJECT'; end;
   try, g.butlabel;       catch, g.butlabel   = 'Interpolation'; end;
  ...
  if isstr(g.eloc_file) | isstruct(g.eloc_file)  % Read in electrode names
      ...
      eeginplot('setelect', g.eloc_file, ax1);
  end;
...
%   %***************************************************
% interpolation_command = 'Win_chan_select = WinRej;';   % the nes line colapse the window after presisng the button
interpolation_command = ['WinRej;', ...     % Call your external function
                         'close(gcbf);'];    % Close the current figure
                     
	  u(12) = uicontrol('Parent',figh, ...
						'Units', 'normalized', ...
						'Position',posbut(12,:), ...
						'Tag','Interpolation',...
						'string',g.butlabel, 'callback', interpolation_command);
%   end;  %*******************************************************
...
%  set(figh, 'busyaction', 'cancel');
  ...
  eeginplot('scaleeye', [], gcf);
  if strcmp(lower(g.scale), 'off')
	  eeginplot('scaleeye', 'off', gcf);
  end;
...
%          end
    ...
    set(ax1, 'ylim',[g.elecoffset*g.spacing (g.elecoffset+g.dispchans+1)*g.spacing] );
	 ...
	 if g.children ~= 0
		...
		eeginplot( 'drawp', p1, p2, g.children);
		figure(figh);
	 end;	  
...
{ 'Number of channels to display:' } , 'Change number of channels to display', 1,  { num2str(g.dispchans) });
   ...
   set(gcf, 'UserData', g);
   eeginplot('updateslider', fig);
   eeginplot('drawp',0);	
   eeginplot('scaleeye', [], fig);
   return;
  ...
  case 'loadelect' % load channels
	...
	eeginplot('setelect',[ inputpath inputname ],AXH0);
	return;
  ...
  case 'noui'
      if ~isempty(varargin)
          eeginplot( varargin{:} ); fig = gcf;
      else 
          fig = findobj('tag', 'EEGINPLOT');
      end;
    ...
    if strcmp(get(fig, 'SelectionType'),'normal');
        ...
        if g.incallback ~= 1 % interception of nestest calls
            ...
            if (tmppos(1) >= 0) & (tmppos(1) <= highlim),
                ...
                set(fig,'UserData', g);
                eeginplot('drawp', 0);  % redraw background
            end;
        end;
